...
public class NodeHandler {
    ...
    public Pong getPong(int pingNr, int incarnationCounter) {
        ...
        for (NodeInfo nodeInfo : bufferAsList) {
            if (messageSizeCounter > NodeComp.PIGGYBACK_MESSAGE_SIZE) {
                break;
            }
            ...
            if (nodeInfo.getSendCounter() > NodeComp.LAMBDA * Math.max(1, Math.log(Math.max(1, aliveNodes.size())))) {
